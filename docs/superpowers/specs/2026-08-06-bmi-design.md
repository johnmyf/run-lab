...
字段说明：
- `min` / `max`：区间边界，**min 闭区间、max 开区间**（`bmi >= min && bmi < max` 命中）；`null` 表示无下界 / 无上界
  - 如「21.0 ~ 23.5」= `min:21.0, max:23.5`，命中 `21.0 <= bmi < 23.5`
  - 如「23.0 ~ 25.0+」= `min:23.0, max:null`（无上限）
- 半开区间保证相邻档无缝衔接且不重复：BMI 恰等于交界值（18.5 / 24.0 / 28.0 / 17.0 / 16.5 等）时归入下一档
  - 例：男 BMI 17.0 → 世界顶尖精英（不在「需注意」的 `< 17.0` 内）；BMI 18.5 → 正常（非偏瘦）
- `name`：显示名（身体状态使用「偏胖」而非表格中的「超重」，与界面枚举及图例一致）
- `color`：横轴色块 / 图例圆点颜色
- **数组顺序 = 显示顺序（左→右）**，同时作为算法遍历顺序：跑者层级为 需注意身体健康 → 世界顶尖精英 → 大众精英/严肃跑者 → 健康完赛跑者 → 新手/健身跑者
- 重叠区间（如大众精英与健康完赛在 21.0~21.5 重叠）中，**靠前的更高层级先命中**，天然实现「同时满足取更高级」
- 「需注意身体健康」区间 `[null, 17.0)` / `[null, 16.5)` 仅覆盖低于精英下限的 BMI，不与他档重叠
...
findAdultStatus(bmi, adultData)                // 按数组顺序遍历，[min, max) 半开区间命中，返回首个命中
findRunnerLevel(bmi, gender, runnerData)       // 按数组顺序遍历（左→右，层级高→低），[min, max) 命中即返回
...
按数组顺序（左→右：需注意 → 世界顶尖 → 大众精英 → 健康完赛 → 新手）遍历，`[min, max)` 半开区间
第一个命中的即结果。示例：男性 BMI 21.1 同时命中「大众精英(19~21.5)」和「健康完赛(21~23.5)」，
数组靠前者为大众精英（更高级）→ 返回大众精英。

**低于最低标准处理**：男性 BMI < 17.0 / 女性 BMI < 16.5 时命中数组首项「需注意身体健康」
（区间 `[null, 17.0)` / `[null, 16.5)`，max 开区间）。该区间不与任何正常层级重叠，
BMI 恰等于 17.0 / 16.5 时落入「世界顶尖精英」。
...
**有效分段**（颜色交界位置）：数组顺序即显示顺序（左→右），第 i 段视觉区间为
`[ i==0 ? axisMin : categories[i-1].max , categories[i].max ?? axisMax ]`。
交界点 = `categories[i-1].max`（与相邻下档的 `min` 一致）：
- 成人条：18.5 / 24.0 / 28.0
