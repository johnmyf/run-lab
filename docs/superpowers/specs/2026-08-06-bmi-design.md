...
每条结构（自上而下）：
1. **彩色分段条**：4 段等色块，宽度与各段 BMI 区间在坐标轴上的占比成正比
2. **黑色三角形标记**：定位在分段条上方，`left` 按当前 BMI 在坐标轴上的位置计算
3. **临界标签**：每个颜色交界区显示 临界 BMI 数字 + 对应体重（体重 = BMI × 身高(m)²，取整）
4. **图例行**：彩色圆点 + 名称
...
**低于最低标准处理**（已确认）：男性 BMI < 17.0 / 女性 BMI < 16.5 时，无任何区间命中，
回退为「世界顶尖精英」（数组首项），图形上归入最左段。
...
**有效分段**（颜色交界位置）：第 i 段视觉区间为
`[ i==0 ? axisMin : categories[i-1].max , categories[i].max ?? axisMax ]`。
交界点 = `categories[i].max`（成人条天然相邻：18.5 / 24.0 / 28.0；跑者条为高一级上限：男 19.0 / 21.5 / 23.5，女 18.5 / 20.5 / 22.5）。
